...
describe("Appointments Routes", () => {
  ...
  describe("GET /appointments/availableVets", () => {
    ...
    it("should handle error when fetching available vets", async () => {
      (AppointmentsController.getAvailableVets as jest.Mock).mockRejectedValue(new Error("Error fetching"));
  ...
  describe("GET /appointments/getAppointmentsForVet", () => {
    ...
    it("should handle error fetching vet appointments", async () => {
      (AppointmentsController.getAppointmentsForVet as jest.Mock).mockRejectedValue(new Error("Error fetching"));
  ...
  describe("GET /appointments/getAppointmentsForUser", () => {
    ...
    it("should handle error fetching user appointments", async () => {
      (AppointmentsController.getAppointmentsForUser as jest.Mock).mockRejectedValue(new Error("Error fetching"));
  ...
  describe("POST /appointments/bookAppointment", () => {
    ...
    it("should handle error when booking appointment", async () => {
      (AppointmentsController.bookAppointment as jest.Mock).mockRejectedValue(new Error("Error booking"));

      const res = await request(app).post("/appointments/bookAppointment").send({ user_id: "user1" });
  ...
  describe("POST /appointments/respondAppointment", () => {
    ...
    it("should handle error when responding to appointment", async () => {
      (AppointmentsController.respondAppointment as jest.Mock).mockRejectedValue(new Error("Error responding"));

      const res = await request(app).post("/appointments/respondAppointment").send({ appointment_id: "appt1", status: "rejected" });
